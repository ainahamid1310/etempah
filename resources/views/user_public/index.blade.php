        ...
        <table class="table datatable-basic table-hover" style="font-size:13px; font-family:arial">
            ...
            <tbody>
                @foreach ($users as $user)
                    ...
                    <a href="/user/show/{{ $user->id }}"><span class="badge bg-teal"><i class="icon-list3"></i></span></a>
                    <a href="/user/edit/{{ $user->id }}"><span class="badge bg-blue-800"><i class="icon-pencil"></i></span></a>
                    <a href="#" onclick="message({{ $user->id }})"><span class="badge badge-warning"><i class="icon-trash"></i></span></a>
                </td> --}}

                        <td class="text-center">
                            <form class="delete" method="POST" action="/user">
                                <a href="/user/show/{{ $user->id }}"><button type="button"
                                        class="btn btn-outline bg-primary-400 text-primary-800 btn-icon rounded-round"><i
                                            class="icon-eye"></i></button>
                                    <a href="/user/edit/{{ $user->id }}"><button type="button"
