...
@section('content')
            ...
            <table class="table table-bordered table-hover datatable-highlight">
                ...
                <thead>
                    <tr>
                        ...
                        <th style="width: 21%">Nama Mesyuarat</th>
                        ...
                        <th style="width: 20%" class="text-center">Aduan</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($reports as $report)
                        <tr>
                            ...
                            <td><a href="/admin/report/show/{{ encrypt($report->id) }}">{{ $report->nama_mesyuarat }}</a>
                            </td>
                            <td>{{ $report->application->room->nama }}</td>
                            <td>{{ date('d-m-Y', strtotime($report->tarikh_mula)) }} -
                                {{ date('d-m-Y', strtotime($report->tarikh_hingga)) }}
                                <br>({{ date('h:i A', strtotime($report->tarikh_mula)) }} -
                                {{ date('h:i A', strtotime($report->tarikh_hingga)) }})
                            </td>
                            <td>
                                @if (!empty($report->application->applicationRoom))
                                    {{ $report->application->applicationRoom->statusRoom->status_pemohon }}
                                @endif

                            </td>
                            <td>
                                @if (!empty($report->application->applicationVc))
                                    {{ $report->application->applicationVc->statusVc->status_pemohon }}
                                @endif
                            </td>
                            ...
                            <td>
                                <p class="text-center"><button type="button" class="btn btn-success" data-toggle="modal"
                                        data-target="#modal_aduan-{{ $report->id }}"><i
        ...
        <div id="modalBatal_room_vc" class="modal fade" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="/admin/application_room/result/{{ $report->id }}" method="post"
    ...
    @foreach ($reports as $report)
        <div id="modal_aduan-{{ $report->id }}" class="modal fade" tabindex="-1">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea rows="2" cols="3" class="form-control" placeholder="Aduan" name="aduan" readonly>{{ $report->aduan }}</textarea>
                                <input type="hidden" name="applicationId" value="{{ old('applicationId', $report->id) }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea rows="2" cols="3" class="form-control" placeholder="Cadangan Penambahbaikan" name="cadangan"
                                    readonly>{{ $report->cadangan }}</textarea>
                            </div>
                        </div>
                    </div>
