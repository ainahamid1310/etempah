                                                        ...
                                                        <tr class="booking-row align-middle">
                                                            ...
                                                            <td>
                                                                <div class="input-group">
                                                                    <input type="text"
                                                                        ...
                                                                        value="{{ $booking['start'] ?? '' }}"
                                                                        placeholder="Pilih tarikh mula" @if(($booking['status_room_id'] ?? null) == 13) disabled @endif>
                                                                    <span class="input-group-text">
                                                            ...
                                                            <td>
                                                                <div class="input-group">
                                                                    <input type="text"
                                                                        ...
                                                                        @if(($booking['status_room_id'] ?? null) == 13) disabled @endif>
                                                                    <span class="input-group-text">
                                                                        <i class="fas fa-calendar-alt"></i>
                                                                    </span>
                                                                </div>
                                                            ...
                                                            <td class="text-center">
                                                                @if ($booking['status_room_id'] == 13)
                                                                    <button type="button"
                                                                            class="btn btn-warning btn-sm btn-toggle-status" style="font-size: 0.6rem; padding: 2px 6px;"
                                                                            data-room-id="{{ $booking['id'] }}" title="Klik untuk tukar status BARU">
                                                                        Batal oleh Pentadbir
                                                                    </button>
                                                                @elseif ($booking['status_room_id'] == 1)
                                                                    <button type="button"
                                                                            class="btn btn-success btn-sm btn-toggle-status"
                                                                            style="font-size: 0.6rem; padding: 2px 6px;"
                                                                            data-room-id="{{ $booking['id'] }}" title="Klik untuk tukar status BATAL">
                                                                        Baru
                                                                    </button>
                                                                @endif
                                                            </td>
                                                            ...
                                                            <td>
                                                                <div class="input-group">
                                                                    <input type="text"
                                                                        ...
                                                                        class="form-control start-input"
                                                                        value=""
                                                                        placeholder="Pilih tarikh mula" @if($booking['status_room_id'] == 13) readonly @endif>
                                                            ...
                                                            <td>
                                                                <div class="input-group">
                                                                    <input type="text"
                                                                        ...
                                                                        class="form-control end-input"
                                                                        value=""
                                                                        placeholder="Pilih tarikh tamat" @if($booking['status_room_id'] == 13) readonly @endif>
                                                                    <span class="input-group-text"><i class="fas fa-calendar-alt"></i></span>
                                                                </div>
                                                            </td>
...
<script>
    ...
    function markInvalidSequenceRows() {
        ...
        // ✅ Parser tarikh + masa (format: dd/mm/yyyy HH:mm)
        ...
        rows.forEach((r, idx) => {
            ...
            if (s && e) {
                const startDate = parseDateTime(s);
                const endDate = parseDateTime(e);
        ...
        // ❌ Semak urutan sequential (guna masa penuh)
        for (let i = 1; i < bookings.length; i++) {
            const prevEnd = parseDateTime(bookings[i - 1].end);
            const currStart = parseDateTime(bookings[i].start);
            const row = bookings[i].row;
            const statusSpan = row.querySelector('.availability-status');

            if (!prevEnd || !currStart) continue;

            // Tarikh mula mesti selepas tarikh tamat sebelumnya
            if (currStart <= prevEnd) {
                console.log("Semak susunan tarikh...problem");
            ...
            // 🎯 Kawal butang
        const btnPindaan = document.querySelector('.btn-lulus-pindaan');
        if (btnPindaan) {
            btnPindaan.disabled = sequenceHasError;
        }
        }

        

        // ✅ Jika tiada ralat susunan, terus semak ketersediaan untuk setiap baris
        if (!sequenceHasError) {
            console.log("Semua susunan betul. Menjalankan checkAvailability() bagi setiap baris...");

            rows.forEach((r, index) => {
                const start = r.querySelector('.start-input')?.value;
                const end = r.querySelector('.end-input')?.value;
                const roomId = document.getElementById('room')?.value;

                console.log(`🔍 Baris ke-${index + 1}: roomId=${roomId}, start=${start}, end=${end}`);
                console.log(`🧩 Debug Baris ${index+1}`, { roomId, start, end });

                if (roomId && start && end && typeof checkAvailability === 'function') {
                    CheckAvailability(roomId, start, end, r);
                } else {
                    console.warn(`⚠️ Baris ke-${index + 1} — maklumat tidak lengkap, semakan dibatal.`);
                }
            });
        }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('addRow').addEventListener('click', function () {
            ...
            // Buang attribute 'disabled' supaya baris baru aktif
            input.removeAttribute('disabled');
            input.classList.remove('error-border');

            // Ubah index [i] pada name
            const name = input.getAttribute('name');
            if (name) input.setAttribute('name', name.replace(/\[\d+]/, `[${index}]`));

            // Buang id (supaya unique)
            input.removeAttribute('id');

            // Reset Flatpickr jika wujud
            if (input._flatpickr) input._flatpickr.destroy();
        });

        // 2️⃣ Kosongkan teks batch_id dan id
        newRow.querySelectorAll('span.input-group-text').forEach(span => {
            span.textContent = ''; // kosongkan ID & Batch ID
        });

        // 3️⃣ Kosongkan status paparan
        newRow.querySelectorAll('.availability-status').forEach(span => {
            span.textContent = '';
        });

        // 4️⃣ Kosongkan sel terakhir (status atau butang)
        ...
        // 5️⃣ Tambah row baru ke dalam tbody
        bookingContainer.appendChild(newRow);

        // 6️⃣ Inisialisasi semula Flatpickr / event
        ...
        // 🗑️ Event listener untuk padam row
...
<script>
    $(document).on('click', '.btn-toggle-status', function () {
        const button = $(this);
        const id = button.data('room-id');
        const row = button.closest('tr'); // Tambah ini sebelum guna row.find()

        Swal.fire({
            title: 'Mesej Pengesahan',
            text: 'Anda pasti mahu menukar status tempahan ini?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/application_room/edit_approve/${id}`,
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
                    success: function (response) {
                        if (response.success) {
                            if (response.toggled === 'cancelled') {
                                // 🚫 Status dibatalkan → tukar kepada kuning
                                button.removeClass('btn-success').addClass('btn-warning');
                                button.text('Batal oleh Pentadbir');
                                row.find('.start-input, .end-input').prop('disabled', true);
                            } else if (response.toggled === 'un-cancelled') {
                                // ✅ Aktif semula → tukar kepada hijau
                                button.removeClass('btn-warning').addClass('btn-success');
                                button.text('Baru');
                                row.find('.start-input, .end-input').prop('disabled', false);
                            }
                            ...
                            Swal.fire({
                                ...
                                // 🔄 Refresh page selepas SweetAlert tutup
                                location.reload();
                            });
                        } else {
                    ...
                    error: function () {
                        Swal.fire('Ralat', 'Ralat semasa memproses permintaan.', 'error');
                    }
                });
            }
        });
...
<!-- Sweetalert Submit Kelulusan -->
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const btnLulus = document.querySelector('.btn-lulus-pindaan');

        if (btnLulus) {
            btnLulus.addEventListener('click', function (e) {
                e.preventDefault(); // elak terus submit

                Swal.fire({
                    title: 'Sahkan Kelulusan?',
                    text: 'Anda pasti ingin meluluskan permohonan ini dengan pindaan?',
                    // icon: 'warning',
                    showCancelButton: true,
