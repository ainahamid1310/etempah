...
@section('breadcrumb')
    <div class="breadcrumb">
        ...
        <span class="breadcrumb-item active">Maklumat Pentadbir</span>
    </div>
@endsection
            ...
            <div>
                                                ...
                                                @if (!empty($user->profile))
                                                    @foreach ($positions as $position)
                                                        <option value="{{ $position->id }}"
                                                            @if (old('position', $user->profile->position_id) == $position->id) selected @endif>
                ...
                <div class="card mb-0 rounded-0 border-y-0">
                    <div class="card-header">
                        ...
                        @if (empty($supervisorsRooms))
                            <span class="text-muted text-center"><i>- Tiada -<i></span>
                        @endif
                    </div>
                    ...
                    <div id="collapsible-item-group2" class="collapse show active">
                        <div class="card-body">
                                ...
                                <select name="rooms[]" data-placeholder="Pilih Bilik" multiple="multiple"
                                    ...
                                    @foreach ($user->rooms as $room)
                                        <option value="{{ $room->id }}"
                                            @if (old('rooms', $room->id) == $room->id) selected @endif
                                            style="background-color: #00ff00;">{{ $room->nama }}</option>
                                    @endforeach
                                </select>
                            </p>
                            ...
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group row mb-0">
                                    <div class="col-md-6 offset-md-4">
                                        <a href="/admin/edit/{{ encrypt($user->id) }}"><button type="button"
                                                class="btn btn-success">Kemaskini <i
                                                    class="icon-pencil ml-2"></i></button></a>
